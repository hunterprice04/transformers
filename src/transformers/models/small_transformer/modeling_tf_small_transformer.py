...
""" TF 2.0 Small Transformer model."""
...
# Copied from transformers.models.gpt2.modeling_tf_gpt2.TFAttention with GPT2->SmallTransformer
...
@keras_serializable
# Copied from transformers.models.gpt2.modeling_tf_gpt2.TFGPT2MainLayer with GPT2->SmallTransformer
class TFSmallTransformerMainLayer(tf.keras.layers.Layer):
...
# Copied from transformers.models.gpt2.modeling_tf_gpt2.TFGPT2PreTrainedModel with GPT2->SmallTransformer
class TFSmallTransformerPreTrainedModel(TFPreTrainedModel):
    """
...
@dataclass
# Copied from transformers.models.gpt2.modeling_tf_gpt2.TFGPT2DoubleHeadsModelOutput with GPT2->SmallTransformer
class TFSmallTransformerDoubleHeadsModelOutput(ModelOutput):
    """
...
class TFSmallTransformerModel(TFSmallTransformerPreTrainedModel):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        self.transformer = TFSmallTransformerMainLayer(config, name="transformer")
...
class TFSmallTransformerLMHeadModel(TFSmallTransformerPreTrainedModel, TFCausalLanguageModelingLoss):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        self.transformer = TFSmallTransformerMainLayer(config, name="transformer")
    ...
    def prepare_inputs_for_generation(self, inputs, past=None, use_cache=None, use_xla=False, **kwargs):
        # TODO: (Joao) after the TF generator is complete, update SMALL_TRANSFORMER TF generation to match PT's. NB -- some SMALL_TRANSFORMER
...
@add_start_docstrings(
    """
    The SMALL_TRANSFORMER Model transformer with a language modeling and a multiple-choice classification head on top e.g. for
...
class TFSmallTransformerDoubleHeadsModel(TFSmallTransformerPreTrainedModel):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        config.num_labels = 1
        self.transformer = TFSmallTransformerMainLayer(config, name="transformer")
    ...
    @unpack_inputs
    @add_start_docstrings_to_model_forward(SMALL_TRANSFORMER_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=TFSmallTransformerDoubleHeadsModelOutput, config_class=_CONFIG_FOR_DOC)
    def call(
        self,
        input_ids: Optional[TFModelInputType] = None,
        past: Optional[Tuple[Tuple[Union[np.ndarray, tf.Tensor]]]] = None,
        ...
        return_dict: Optional[bool] = None,
        training: Optional[bool] = False,
    ) -> Union[TFSmallTransformerDoubleHeadsModelOutput, Tuple[tf.Tensor]]:
        r"""
        ...
        ```python
        >>> import tensorflow as tf
        >>> from transformers import GPT2Tokenizer, TFSmallTransformerDoubleHeadsModel

        >>> tokenizer = GPT2Tokenizer.from_pretrained("small_transformer")
        >>> model = TFSmallTransformerDoubleHeadsModel.from_pretrained("small_transformer")
        ...
        return TFSmallTransformerDoubleHeadsModelOutput(
            logits=lm_logits,
            mc_logits=mc_logits,
            past_key_values=transformer_outputs.past_key_values,
            hidden_states=transformer_outputs.hidden_states,
...
@add_start_docstrings(
    ...
    [`TFSmallTransformerForSequenceClassification`] uses the last token in order to do the classification, as other causal models
    (e.g. GPT-1) do.
...
class TFSmallTransformerForSequenceClassification(TFSmallTransformerPreTrainedModel, TFSequenceClassificationLoss):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
